...
function renderSocial() {
  ...
  row.innerHTML = SOCIAL_LINKS.map(s => {
    const platformClass = `social-${s.platform.toLowerCase()}`;
    return `
    <a href="${s.url}" target="_blank" rel="noopener" aria-label="${s.platform}" class="${platformClass}">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" fill-rule="evenodd"><path d="${s.icon}"/></svg>
    </a>
  `;
  }).join('');
}
